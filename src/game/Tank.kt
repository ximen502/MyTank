...
//        }
    ...
    override fun begin(direction: Int) {
        println("tank got begin ${direction}")
    }

    override fun end(direction: Int) {
        println("tank got end ${direction}")
        if (direction == KeyEvent.VK_UP) {
            println("tank's y: ${y}, y对齐 = ${y - y % 25}")
            y -= y % 25
        } else if (direction == KeyEvent.VK_DOWN) {
            println("tank's y: ${y}, y对齐 = ${y - y % 25 + 25}")
            y = y - y % 25 + 25
        } else if (direction == KeyEvent.VK_LEFT) {
            println("tank's x: ${x}, x对齐 = ${x - x % 25}")
            x -= x % 25
        }  else if (direction == KeyEvent.VK_RIGHT) {
            println("tank's x: ${x}, x对齐 = ${x - x % 25 + 25}")
            x = x - x % 25 + 25
        }
    }
}